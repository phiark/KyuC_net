...
from frcnet.evaluation.matched_manifest import compute_manifest_hash
...
def test_state_entropy_decomposition_matches_contract():
    ...
    assert torch.allclose(
        metrics.state_weighted_content_entropy,
        resolution_ratio * metrics.state_content_entropy,
    )
    assert torch.all(metrics.state_entropy >= metrics.state_weighted_content_entropy)
...
def test_primary_scalar_whitelist_excludes_label_aware_proposition_fields():
    assert "proposition_truth_ratio" not in LABEL_FREE_SCALAR_FEATURES
    assert "proposition_truth_mass" not in LABEL_FREE_SCALAR_FEATURES
    ...
    for repeat in range(2):
        for record in sample_records:
            duplicated_records.append(replace(record, sample_id=f"{record.sample_id}-{repeat}"))
...
def test_matched_manifest_hash_is_stable_and_sensitive_to_reference_score():
    ...
    first_hash = compute_manifest_hash(records)
    assert first_hash == compute_manifest_hash([replace(record, manifest_hash=first_hash) for record in records])
    ...
    assert compute_manifest_hash(changed_records) != first_hash
