...
def test_frozen_matched_manifest_pairs_roles_and_bin_diagnostics(tmp_path: Path):
    ...
    manifest_records, diagnostics = build_frozen_matched_manifest_with_diagnostics(
        samples,
        references,
        num_bins=2,
        test_size=0.5,
...
def test_frozen_manifest_hash_changes_when_reference_score_changes():
    samples, references = _fixture_records()
    first_records, _ = build_frozen_matched_manifest_with_diagnostics(samples, references, num_bins=2, random_state=7)
    changed_references = list(references)
    changed_references[0] = replace(changed_references[0], reference_score_value=0.33)
    changed_records, _ = build_frozen_matched_manifest_with_diagnostics(
        samples,
        changed_references,
        num_bins=2,
        random_state=7,
...
def test_scalar_scan_uses_same_frozen_manifest_subset_as_pair_benchmark():
    ...
    manifest_records, _ = build_frozen_matched_manifest_with_diagnostics(
        samples,
        references,
        num_bins=2,
        test_size=0.5,
